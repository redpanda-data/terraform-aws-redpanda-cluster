...
resource "aws_route_table_association" "connect" {
  subnet_id      = aws_subnet.connect.id
  route_table_id = aws_route_table.test.id
}
...
resource "aws_security_group" "connect" {
  name        = "${var.deployment_prefix}-connect-sg"
  description = "connect security group"
  tags        = var.tags
  vpc_id      = aws_vpc.test.id
  ingress {
  ...
  ingress {
    description     = "Allow into Connect"
    from_port       = 8083
    to_port         = 8083
    protocol        = "tcp"
    cidr_blocks     = ["0.0.0.0/0"]
...
module "redpanda-cluster" {
  ...
  security_groups_connect         = [aws_security_group.connect.id]
  subnets                         = {
    broker = {
  ...
  egress_rules      = {
    ...
    "CONNECT" = {
      description     = "Allow connect security group"
      from_port       = 8083
      to_port         = 8083
      protocol        = "tcp"
      enabled         = true
      cidr_blocks     = [aws_subnet.connect.cidr_block]
      self            = null
  ...
  ingress_rules = {
    ...
    "Kafka" = {
      ...
      cidr_blocks     = [aws_subnet.client.cidr_block]
      self            = null
      security_groups = [aws_security_group.client_sec_group.id, aws_security_group.connect.id]
    }
    "RPC" = {
